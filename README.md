# Actividades de Figuras y Vehículos

Este proyecto contiene dos actividades realizadas en Java. La primera trata sobre figuras geométricas y la segunda sobre vehículos utilizando herencia y clases abstractas.

## Actividad 1: Figuras

Se crean dos clases que implementan una interfaz común:

- Rectángulo
- Círculo

El programa pide los datos por consola y muestra el área y el perímetro de cada figura.

## Actividad 2: Vehículos

Se crea una clase abstracta para representar un vehículo y una clase concreta para un coche de combustión.  
El programa crea un coche, intenta moverlo antes de arrancar, lo arranca y lo mueve varias veces mostrando el consumo de combustible.

## Ejecución

El archivo `App.java` contiene el método main que ejecuta ambas actividades de forma sencilla y directa.
